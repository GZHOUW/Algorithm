...
class Solution:
    def findMinArrowShots(self, points):
        ...
        if L < 2:
            return L
        points.sort()
        arrows = L # start with max number of arrows (one for each point)
        curr_end = points[0][1]
        for i in range(1, L):
            if points[i][0] <= curr_end: # overlapping found
                arrows -= 1 # one less arrow needed
                curr_end = min(curr_end, points[i][1]) # keep the smaller end bc. there can be more than 2 overlaps
            else:
                curr_end = points[i][1] # non overlapped, update end
        return arrows
